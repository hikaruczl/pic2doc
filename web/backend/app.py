...
async def process_merged_task(task_id: str):
    """
    后台处理合并任务
    ...
    try:
        ...
        task["progress"] = 30
        task["message"] = "正在调用LLM分析多个图像..."

        # 处理所有图像并合并
        result = ocr.process_batch_merged(
            task["file_paths"],
            output_filename=f"merged_{task_id}.docx"
        )

        if result['success']:
            task["status"] = "completed"
            task["progress"] = 100
            task["message"] = f"处理完成，已合并 {result.get('images_processed', 0)} 个图像"
            task["result"] = {
                "output_path": result['output_path'],
                "statistics": result['statistics'],
                "provider": result['analysis']['provider'],
...
async def batch_process(
    ...
    try:
        ...
        if merge_documents:
            ...
            tasks[batch_id] = task
            asyncio.create_task(process_merged_task(batch_id))

            return {
                "batch_id": batch_id,
                "total_tasks": 1,
                "merge_mode": True,
                "message": "合并任务已提交，正在处理中"
            }
        else:
            # 分离模式：为每个文件创建独立任务
            sub_task_ids = []
...
async def get_batch_status(batch_id: str, current_user: str = Depends(get_current_user)):
    ...
    """
    # 先检查是否是合并任务
    merge_task = tasks.get(batch_id)
    if merge_task and merge_task.get("config", {}).get("merge_documents"):
        # 合并模式：返回单个任务的状态
        if merge_task.get("user") != current_user:
            raise HTTPException(status_code=403, detail="无权访问该任务")
        ...
        return {
            ...
            "merge_mode": True,
            "total": 1,
            "completed": 1 if merge_task["status"] == "completed" else 0,
            "failed": 1 if merge_task["status"] == "failed" else 0,
            "processing": 1 if merge_task["status"] == "processing" else 0,
            "pending": 1 if merge_task["status"] == "pending" else 0,
            "tasks": [{
                "task_id": batch_id,
                "status": merge_task["status"],
                "message": merge_task.get("message", ""),
                "file_path": "merged_document",
                "result": merge_task.get("result")
            }]
        }
